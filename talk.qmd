...
Instead of relying on algorithmic heuristics (e.g., IC3 or Interpolation) to guess invariants, the system treats generalization as a native categorical operation over its Reference Theory ($S_F$). We reframe invariant proposal through the foundational duality of fibrations and adjunctions.
...
* **2. Adjoint Duality (Swapping the Problem)**
  * Generalizing a safe invariant is structurally identical to the problem of deductive planning, simply with the functors reversed (an Adjunction: $Left \dashv Right$).
  * *Planning (Left Adjoint):* Inductive, bottom-up synthesis of a finite trace (Least Fixpoint).
  * *Generalizing (Right Adjoint):* Coinductive, top-down projection of an infinite bound (Greatest Fixpoint). The finite local observations mathematically push across the adjunction to form the exact coinductive proposal.
