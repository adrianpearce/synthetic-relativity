...
*   **Strict Tree Topology ([Unique Predecessors]{style="color:blue"}):** Distinct action histories never converge. Even if two different paths result in the same state, the logic traps them in completely isolated parallel universes.
...
*   **The Finiteness Barrier:** By mathematical definition, a least fixpoint ($\mu$) only constructs strictly *finite* chains. [It is fundamentally blind to continuous processes, nn
:::
...
To reason about infinite streams when operating in [coalgebras]{style="color:blue"}, we harness two categorical proof mechanisms.
...
Following $+2$, the stream outputs a jump of $-1$. How does our *proposed invariant* survive this paradox? 
